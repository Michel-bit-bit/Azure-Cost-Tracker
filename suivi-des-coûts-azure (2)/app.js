document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculateTotal(entry) {
        if (!entry) return 0;
        return CATEGORY_KEYS.reduce((sum, key) => sum + (Number(entry[key]) || 0), 0);
    }
    ...
    function renderDonutChart(canvasId, legendId, entry, compareEntry) {
        ...
        const chartData = {
            labels: CATEGORY_KEYS.map(key => COST_CATEGORIES[key].label),
            datasets: [{
                data: CATEGORY_KEYS.map(key => entry[key] || 0),
                backgroundColor: CATEGORY_KEYS.map(key => getCssVar(COST_CATEGORIES[key].color)),
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                let lines = [
                                    `${label}: ${Number(value).toFixed(2)}€ (${percentage}%)`,
                                    `Part du total: ${percentage}%`
                                ];
                                
                                if (compareEntry) {
                                    const categoryKey = CATEGORY_KEYS[context.dataIndex];
                                    const compareValue = compareEntry[categoryKey] || 0;
                                    const diff = value - compareValue;
                                    const diffSign = diff >= 0 ? '+' : '';
                                    lines.push(`Diff. vs mois comparé: ${diffSign}${diff.toFixed(2)}€`);
                                }
                                return lines;
                            }
                        }
        ...
        legendContainer.innerHTML = CATEGORY_KEYS.map(key => `
            <li class="legend-item">
                <span class="legend-color-box" style="background-color:${getCssVar(COST_CATEGORIES[key].color)}"></span>
        ...
        return `
            <form id="entry-form">
                ...
                <div style="text-align: right; margin-top: 1.5rem; font-size: 1.2rem;">
                    <strong>Prix TTC : <span id="total-ttc">0.00</span> €</strong>
                </div>
