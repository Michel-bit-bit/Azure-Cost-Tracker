...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const CATEGORY_KEYS = Object.keys(COST_CATEGORIES);
    const TAX_RATE = 1.20; // 20% TVA
    ...
    let state = {
        ...
        searchTerm: '',
        comparison: { 
            year1: null, month1: null, 
            year2: null, month2: null 
        },
    ...
    function calculateTotal(entry) {
        if (!entry) return 0;
        // Le total TTC est soit la valeur explicite, soit la somme calculée (fallback)
    ...
    const db = {
        ...
        init() {
            return new Promise((resolve, reject) => {
                // Version 2 pour ajouter le store 'notes'
                const request = indexedDB.open('AzureCostTrackerDB_v1', 2);
        ...
        addEntry(entry) {
            return new Promise((resolve, reject) => {
                const request = this.getObjectStore('costEntries', 'readwrite').add(entry);
        ...
        updateEntry(entry) {
            return new Promise((resolve, reject) => {
                const request = this.getObjectStore('costEntries', 'readwrite').put(entry);
                request.onsuccess = () => resolve();
                request.onerror = (e) => reject(e.target.error);
            });
        },
        deleteEntry(id) {
            return new Promise((resolve, reject) => {
                const request = this.getObjectStore('costEntries', 'readwrite').delete(id);
                request.onsuccess = () => resolve();
                request.onerror = (e) => reject(e.target.error);
            });
        },
        getAllEntries() {
            return new Promise((resolve, reject) => {
                const request = this.getObjectStore('costEntries', 'readonly').getAll();
        ...
        addNote(note) {
            return new Promise((resolve, reject) => {
                const request = this.getObjectStore('notes', 'readwrite').add(note);
                request.onsuccess = () => resolve();
                request.onerror = (e) => reject(e.target.error);
            });
        },
        getNotes() {
            return new Promise((resolve, reject) => {
                const request = this.getObjectStore('notes', 'readonly').getAll();
                request.onsuccess = e => resolve(e.target.result || []);
                request.onerror = (e) => reject(e.target.error);
            });
        },
        updateNote(note) {
            return new Promise((resolve, reject) => {
                const request = this.getObjectStore('notes', 'readwrite').put(note);
                request.onsuccess = () => resolve();
                request.onerror = (e) => reject(e.target.error);
            });
        },
        deleteNote(id) {
            return new Promise((resolve, reject) => {
                const request = this.getObjectStore('notes', 'readwrite').delete(id);
                request.onsuccess = () => resolve();
                request.onerror = (e) => reject(e.target.error);
            });
        },
        // --- Import/Export ---
        exportData() {
            return Promise.all([this.getAllEntries(), this.getNotes()]).then(([entries, notes]) => {
        ...
        importData(jsonString) {
            return new Promise((resolve, reject) => {
                try {
                    ...
                    const transaction = this._db.transaction(['costEntries', 'notes'], 'readwrite');
                    
                    transaction.oncomplete = () => resolve();
                    transaction.onerror = (e) => reject(e.target.error);

                    const entryStore = transaction.objectStore('costEntries');
                    entryStore.clear().onsuccess = () => {
                        entries.forEach(entry => {
                            const { id, ...entryWithoutId } = entry;
                            entryStore.add(entryWithoutId);
                        });
                    };
    ...
    function adjustEntriesListHeight() {
        if (window.innerWidth < 1024) {
            entriesContainer.style.height = '';
            return;
        }
        const comparisonHeight = comparisonContainer.offsetHeight;
        // S'assurer que la hauteur est au moins suffisante pour afficher le contenu de base
        entriesContainer.style.height = Math.max(comparisonHeight, 400) + 'px';
    }
    ...
    function render() {
        ...
        renderAnnualReview();
        // On attend un cycle pour que le DOM se mette à jour avant d'ajuster la hauteur
        setTimeout(adjustEntriesListHeight, 50);
    }
    ...
    function renderComparisonView() {
        const { year1, month1, year2, month2 } = state.comparison;
        const entry1 = state.entries.find(e => e.year === year1 && e.month === month1);
        const entry2 = state.entries.find(e => e.year === year2 && e.month === month2);
        ...
        const yearOptions = (selectedYear) => availableYears.map(y => `<option value="${y}" ${y === selectedYear ? 'selected' : ''}>${y}</option>`).join('');
        const monthOptions = (selectedMonth) => MONTHS.map((m, i) => `<option value="${i}" ${i === selectedMonth ? 'selected' : ''}>${m}</option>`).join('');
        ...
        comparisonContainer.innerHTML = `
            ...
            <div class="comparison-selectors">
                <div class="selector-group">
                    <select id="compare-month-1" class="form-select">${monthOptions(month1)}</select>
                    <select id="compare-year-1" class="form-select">${yearOptions(year1)}</select>
                </div>
                <div class="selector-group">
                    <select id="compare-month-2" class="form-select">
                        <option value="">-- Mois --</option>
                        ${monthOptions(month2)}
                    </select>
                    <select id="compare-year-2" class="form-select">
            ...
            <div class="charts-container">
                <div>
                    ...
                    ` : '<div class="text-placeholder" style="height: 300px;"><p>Sélectionnez la période 1</p></div>'}
                </div>
                <div>
                    ${entry2 ? `
                        <div class="chart-wrapper">
                    ...
                    ` : '<div class="text-placeholder" style="height: 300px;"><p>Sélectionnez la période 2</p></div>'}
                </div>
            </div>
        `;

        if (entry1) tryRenderDonutChart('donut-chart1', 'donut-chart1-legend', entry1, entry2);
        if (entry2) tryRenderDonutChart('donut-chart2', 'donut-chart2-legend', entry2, entry1);
    }
    
    function tryRenderDonutChart(canvasId, legendId, entry, compareEntry) {
        try {
            renderDonutChart(canvasId, legendId, entry, compareEntry);
        } catch (e) {
            console.error("Erreur d'affichage du graphique:", e);
            const canvas = document.getElementById(canvasId);
            if (canvas) {
                canvas.parentElement.innerHTML = '<p class="text-center text-danger">Erreur de chargement du graphique. Vérifiez votre connexion (Chart.js).</p>';
            }
        }
    }
    ...
    function renderAnnualReview() {
        ...
        if (year && state.entries.filter(e => e.year === year).length > 0) {
            try {
                ...
                console.error("Erreur bar chart:", e);
            }
        }
    }
    ...
    const externalTooltipHandler = (context) => {
        ...
        if (tooltip.body) {
            ...
            let innerHtml = '';
            titleLines.forEach(title => { innerHtml += '<h4>' + title + '</h4>'; });
            bodyLines.forEach((body, i) => { innerHtml += '<p>' + body[0] + '</p>'; });
    ...
    function renderDonutChart(canvasId, legendId, entry, compareEntry) {
        ...
        // Calculs
        // CATEGORY_KEYS inclut 'autre' car il est dans COST_CATEGORIES
        const sumOfParts = CATEGORY_KEYS.reduce((sum, key) => sum + (entry[key] || 0), 0);
        const totalTTC = calculateTotal(entry);
        const unallocated = Math.max(0, totalTTC - sumOfParts);
        ...
        if (unallocated > 0.001) {
            ...
            labels.push("Non alloué");
            colors.push(getCssVar('--chart-c7'));
        }
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            ...
                            label: function(context) {
                                ...
                                if (compareEntry) {
                                    let compareValue = 0;
                                    if (context.label === "Non alloué") {
                                         const compareSum = CATEGORY_KEYS.reduce((sum, k) => sum + (compareEntry[k]||0), 0);
                                         const compareTotal = calculateTotal(compareEntry);
                                         compareValue = Math.max(0, compareTotal - compareSum);
                                    } else {
                                        // Trouver la clé correspondant au label
                                        const categoryEntry = Object.entries(COST_CATEGORIES).find(([k, v]) => v.label === context.label);
                                        if (categoryEntry) {
                                            compareValue = compareEntry[categoryEntry[0]] || 0;
                                        }
                                    }
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                let lines = [`Coût: ${value.toFixed(2)}€`, `Part annuelle: ${percentage}%`];
    ...
    function getAddEditModalContent(entry = null) {
        ...
        const yearToSelect = entry ? entry.year : new Date().getFullYear();
        const yearOptions = years.map(y => `<option value="${y}" ${y === yearToSelect ? 'selected' : ''}>${y}</option>`).join('');
        ...
        return `
            <form id="entry-form">
                ...
                <div class="form-group-checkbox">
                    <input type="checkbox" id="isHT" name="isHT">
                    <label for="isHT">Les montants des catégories sont en HT (TVA 20% sera appliquée)</label>
                </div>
                <hr style="margin: 1.5rem 0; border-color: var(--border-color);">
                <div class="form-group">
                     <label for="totalTTC">Prix TTC (optionnel, calculé si la case HT est cochée)</label>
                 ...
                 <div style="text-align: right; margin-top: 1rem; font-size: 1rem;">
                    <p>Somme des catégories (calculée TTC) : <span id="calculated-total">0.00</span> €</p>
                </div>
    ...
    function getNotesModalContent() {
        // Sécurisation : s'assurer que state.notes est un tableau
        const notes = (state.notes || []).sort((a, b) => new Date(b.date) - new Date(a.date));
        
        let editingNote = null;
        if (state.editingNoteId) {
            editingNote = notes.find(n => n.id === state.editingNoteId);
        }

        const notesListHtml = notes.length > 0 ? notes.map(note => `
            <li class="note-item">
                <span class="note-date">${formatDate(note.date)}</span>
                <div class="note-content">${note.content}</div>
                <div class="note-actions">
                    <button class="edit-note-btn" data-id="${note.id}">Modifier</button>
                    <button class="delete-note-btn" data-id="${note.id}">Supprimer</button>
                </div>
            </li>
        `).join('') : '<p style="text-align:center; color:var(--text-secondary);">Aucune note.</p>';

        return `
            <div id="notes-modal-content">
                <form id="note-form" style="margin-bottom: 2rem;">
                    <div class="form-group">
                        <label for="note-text">${editingNote ? 'Modifier la note' : 'Nouvelle note'}</label>
                        <textarea id="note-text" name="content" class="form-textarea" placeholder="Saisissez votre commentaire ici...">${editingNote ? editingNote.content : ''}</textarea>
                    </div>
                    <div class="form-actions">
                        ${editingNote ? '<button type="button" id="cancel-edit-note" class="btn btn-secondary">Annuler modif.</button>' : ''}
                        <button type="submit" class="btn btn-primary">${editingNote ? 'Mettre à jour' : 'Ajouter'}</button>
                    </div>
                </form>
                <hr style="border-color: var(--border-color);">
                <h3 style="margin-top: 1rem; margin-bottom: 0.5rem; font-size: 1rem; color: var(--text-secondary);">Historique</h3>
                <ul class="notes-list">
                    ${notesListHtml}
                </ul>
            </div>
    ...
    function getInfoModalContent() {
        return `
            ...
            <ul style="list-style-position: inside; margin: 1rem 0;">
                <li><strong>Chart.js (v4.4.2)</strong>: Pour la visualisation des données.</li>
            </ul>
            <div class="form-actions">
    ...
    async function handleFormSubmit(e) {
        ...
        const isHT = formData.has('isHT');
        const sumOfPartsRaw = CATEGORY_KEYS.reduce((sum, key) => {
            return sum + (parseFloat(String(formData.get(key)).replace(',', '.')) || 0);
        }, 0);

        if (isHT) {
            CATEGORY_KEYS.forEach(key => {
                const valueHT = parseFloat(String(formData.get(key)).replace(',', '.')) || 0;
                entryData[key] = valueHT * TAX_RATE;
            });
            entryData.totalTTC = sumOfPartsRaw * TAX_RATE;
        } else {
            CATEGORY_KEYS.forEach(key => {
                entryData[key] = parseFloat(String(formData.get(key)).replace(',', '.')) || 0;
            });
            const totalTTCFromInput = parseFloat(String(formData.get('totalTTC')).replace(',', '.')) || 0;
            entryData.totalTTC = totalTTCFromInput > 0 ? totalTTCFromInput : sumOfPartsRaw;
        }
    ...
    async function handleNoteSubmit(e) {
        ...
        try {
            if (state.editingNoteId) {
                noteData.id = state.editingNoteId;
                await db.updateNote(noteData);
                state.editingNoteId = null;
            } else {
                await db.addNote(noteData);
            }
            state.notes = await db.getNotes();
            openModal("Bloc-notes", getNotesModalContent());
        } catch (err) {
            console.error(err);
    ...
    function updateTotalTTC() {
        ...
        const isHTCheckbox = document.getElementById('isHT');
        if (!calculatedTotalSpan || !totalTTCInput || !isHTCheckbox) return;
        
        const isHT = isHTCheckbox.checked;
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (target.classList.contains('edit-note-btn')) {
            state.editingNoteId = parseInt(target.dataset.id);
            openModal("Bloc-notes", getNotesModalContent());
        }
    });
    ...
    document.addEventListener('change', e => {
        ...
        if (target.id === 'isHT') updateTotalTTC();

        // Comparaison
        let comparisonChanged = false;
        if (target.id === 'compare-year-1') { state.comparison.year1 = parseInt(target.value); comparisonChanged = true; }
        if (target.id === 'compare-month-1') { state.comparison.month1 = parseInt(target.value); comparisonChanged = true; }
        if (target.id === 'compare-year-2') { state.comparison.year2 = target.value ? parseInt(target.value) : null; comparisonChanged = true; }
        if (target.id === 'compare-month-2') { state.comparison.month2 = target.value ? parseInt(target.value) : null; comparisonChanged = true; }
    ...
    document.addEventListener('input', e => {
        ...
        if (target.classList.contains('cost-input') || target.id === 'totalTTC') updateTotalTTC();
    });
    
    document.addEventListener('submit', e => {
        if (e.target.id === 'entry-form') handleFormSubmit(e);
        if (e.target.id === 'note-form') handleNoteSubmit(e);
    });
    
    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', applyTheme);
    window.addEventListener('resize', adjustEntriesListHeight);
    ...
    async function refreshData() {
        try {
            ...
            // Init Comparison defaults
            if (state.entries.length > 0 && !state.comparison.year1) {
                const latestEntry = state.entries[0];
                state.comparison.year1 = latestEntry.year;
                state.comparison.month1 = latestEntry.month;
            }
            if (state.entries.length > 1 && !state.comparison.year2) {
                const secondLatestEntry = state.entries[1];
                state.comparison.year2 = secondLatestEntry.year;
                state.comparison.month2 = secondLatestEntry.month;
            }
    ...
    async function init() {
        ...
        try {
            ...
            document.querySelector('.main-container').innerHTML = "<p style='color:red; text-align:center; margin-top:2rem;'>Erreur critique: Impossible de démarrer la base de données.</p>";
        }
    }
