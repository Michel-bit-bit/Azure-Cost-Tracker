# Master Regenerative Prompt: Azure Cost Tracker Application (V.6)
...
-   **CSS3**: For styling. Use CSS variables for theming and a combination of Grid and Flexbox for layout.
...
-   **Body**:
    ...
    -   `<main>` container with a responsive layout (`.content-grid`).
    -   Three main `div` containers with IDs: `comparison-container`, `entries-container`, and `annual-review-container`. These must be inside the `.content-grid`.
...
-   **Components Styling**:
    -   **Layout**: Use CSS Grid for the main two-column layout on large screens (`.content-grid`).
    -   **Cards**: Background, rounded corners, subtle box-shadow.
    -   **Entries List Card (`#entries-container`)**: Must be styled as a flex column (`display: flex; flex-direction: column;`) and have `overflow: hidden`. Its height must NOT be fixed in CSS but will be set by JavaScript. The list element (`.entries-list`) inside must use `flex-grow: 1` and `overflow-y: auto` to fill the available space and become scrollable. The title and controls must not scroll.
    ...
    -   **Responsiveness**: Use media queries to switch from a single-column to a two-column grid layout at a breakpoint (e.g., 1024px).
...
-   **Layout Synchronization (Crucial)**:
    -   Create a function `adjustEntriesListHeight()` that programmatically sets the height of the `#entries-container` to match the `offsetHeight` of the `#comparison-container`.
    -   This function must be called at the end of every `render()` cycle and also attached to a `window.resize` event listener. This is the definitive mechanism for ensuring the two columns are aligned and the list scrolls correctly.
...
-   **Rendering**:
    ...
    -   These functions read from the `state` object and generate HTML to update the DOM. The `renderEntriesList` function must generate the correct structure for the flexbox-based scrolling behavior (title, controls, then the `<ul>` list).
-   **Modal Logic**:
    -   Create `openModal(title, content)` and `closeModal()` functions.
    -   Create "content generator" functions for each modal type (`getAddEditModalContent`, `getSettingsModalContent`, `getInfoModalContent`). The `getAddEditModalContent` must provide a year selector that goes up to 2050.
-   **Chart Logic**:
...
-   **Event Handling**:
    ...
    -   **Comparison View**: Two pairs of dropdowns (month and year for each) allow selecting the two periods to compare. It defaults to the latest two entries. It shows "mois1 année1 : xxx euro vs mois2 année2 xxx euro -> augmentation/baisse de x %".
...
-   **Initial Load**: `app.js` initializes, applies the theme, connects to IndexedDB, fetches entries, populates the `state`, and calls the main `render()` function, which concludes by calling `adjustEntriesListHeight()`.
