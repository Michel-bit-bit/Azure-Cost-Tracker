...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const CATEGORY_KEYS = Object.keys(COST_CATEGORIES); // Includes 'autre' now
    const TAX_RATE = 1.20; // 20% VAT
    ...
    function adjustEntriesListHeight() {
        // En dessous de 1024px, la disposition est en colonne, donc pas besoin d'ajuster.
        if (window.innerWidth < 1024) {
            entriesContainer.style.height = ''; // Réinitialiser la hauteur
            return;
        }
        const comparisonHeight = comparisonContainer.offsetHeight;
        entriesContainer.style.height = `${comparisonHeight}px`;
    }
    ...
    function render() {
        ...
        renderAnnualReview();
        // Ajuster la hauteur après le rendu de tous les composants
    ...
    function renderDonutChart(canvasId, legendId, entry, compareEntry) {
        ...
        if (unallocated > 0.001) { // Utiliser une petite tolérance pour les erreurs de flottants
                            ...
                            label: function(context) {
                                ...
                                if (compareEntry) {
                                    ...
                                    if (context.label === "Non alloué") {
                                        ...
                                        if (categoryKey) {
                                            compareValue = compareEntry[categoryKey] || 0;
                                        }
                                    }
    ...
    async function handleFormSubmit(e) {
        ...
        if (isHT) {
            // Si HT est coché, le calcul est autoritaire.
            CATEGORY_KEYS.forEach(key => {
                const valueHT = parseFloat(String(formData.get(key)).replace(',', '.')) || 0;
                entryData[key] = valueHT * TAX_RATE; // On stocke les valeurs en TTC
            });
            entryData.totalTTC = sumOfPartsRaw * TAX_RATE;
        } else {
            // Si HT n'est pas coché, la logique précédente s'applique.
            CATEGORY_KEYS.forEach(key => {
    ...
    function updateTotalTTC() {
        ...
        if (isHT) {
            totalTTCInput.value = ''; // Vider la valeur manuelle pour éviter la confusion
    ...
    document.addEventListener('change', e => {
        ...
        // Gérer les changements dans les sélecteurs de comparaison
        let comparisonChanged = false;
        if (target.id === 'compare-year-1') {
            state.comparison.year1 = parseInt(target.value);
            comparisonChanged = true;
        }
        if (target.id === 'compare-month-1') {
            state.comparison.month1 = parseInt(target.value);
            comparisonChanged = true;
        }
        if (target.id === 'compare-year-2') {
            state.comparison.year2 = target.value ? parseInt(target.value) : null;
            comparisonChanged = true;
        }
        if (target.id === 'compare-month-2') {
            state.comparison.month2 = target.value ? parseInt(target.value) : null;
            comparisonChanged = true;
        }
    ...
    async function refreshData() {
        ...
        if (state.entries.length > 1) {
            ...
            state.comparison.year2 = null;
            state.comparison.month2 = null;
        }
