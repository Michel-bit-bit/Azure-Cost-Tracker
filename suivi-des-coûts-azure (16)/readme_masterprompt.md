...
# Master Regenerative Prompt: Azure Cost Tracker Application (V.11)
...
-   **Objet d'entrée de coût (`CostEntry`)**: Une entrée de coût doit être un objet JavaScript avec les propriétés suivantes : `id` (auto-incrémenté par IndexedDB), `month` (nombre, 0-11), `year` (nombre), `networking`, `storage`, `compute`, `management`, `marketplace`, `autre` (tous des nombres), et `totalTTC` (nombre, optionnel).
...
-   Le formulaire doit contenir des champs pour le Mois, l'Année, et toutes les catégories de coûts (`networking`, `storage`, `compute`, `management`, `marketplace`, et **`autre`**).
...
-   **Logique de Soumission (Cruciale)**:
    1.  Si la **case HT n'est PAS cochée**:
        ...
        -   Le `totalTTC` final sauvegardé est la valeur du champ "Prix TTC" si elle est fournie ; sinon, c'est la somme des autres catégories (y compris "Autre").
    2.  Si la **case HT EST cochée**:
        -   La logique est **autoritaire** : le total est **calculé** et non saisi.
        -   Toutes les valeurs de catégories saisies sont considérées HT (y compris "Autre").
...
-   **Logique de la Catégorie "Non alloué"**: Pour chaque graphique, calculer la somme des catégories de coûts détaillées (incluant maintenant "Autre"). Si la propriété `totalTTC` de l'entrée est supérieure à cette somme, afficher la différence comme une catégorie distincte "Non alloué" dans le graphique.
